function [cdf] = chemdepletionfactor(Geochem,ImmobileElementI,ProtolithType,XofLowermost)
%chemloss_factor function
% by R.E. Breunig
% last update: 2020.11.11
%INSERT GENERAL DESCRIPTION/ CITATION FOR CDF. The form of the function here in
% particular is capable of calculating  the chemical depletion factor, CDF, of a mobile
% sample in respect to an immobile element (i). CDF is a unitless
% value. Positive values of CDF indicate bulk depletion while negative
...
if contains(ProtolithType,'TotalLowermost')
    ...
    cdf=1-(iprotolith./(Geochem.(ImmobileElementI)));
    if abs(cdf) >= 2
...
elseif contains(ProtolithType,'LowermostPerHole')
   ...
   for i=1:numel(HoleIDList) 
      ...
      [~,index_inrowsofinterest]=max(Geochem.('AvgDepthBelowGroundSurface_m')(rowsofinterest)); 
       ...
       cdf(min(rowsofinterest):max(rowsofinterest),:)=1-(iprotolith./Geochem.(ImmobileElementI)(min(rowsofinterest):max(rowsofinterest))); 
